...
export default function POS() {
  ...
  useEffect(() => {
    if (items.length > 0) {
      api.post('/cart/calculate', { items: items.map(i => ({ product_id: i.product_id, quantity: i.quantity, price: i.price })), customer_id: customerId })
        .then(res => { setSubtotal(res.data.subtotal); setAppliedDiscounts(res.data.discounts || []); })
        .catch(err => console.error(err));
    } else { setSubtotal(0); setAppliedDiscounts([]); }
  }, [items, customerId]);
  useEffect(() => {
    if (customerId) {
  ...
  const searchCustomer = async () => {
    ...
    try {
      ...
      if (found) {
        ...
        setCustomer(null);
        toast.error('Customer not found. Continue as walk-in.');
      }
  ...
  const applyRedeem = () => {
    ...
    if (discount > finalTotalBeforePoints) { toast.error('Discount exceeds total'); return; }
    setDiscountFromPoints(discount);
    toast.success(`${redeemPoints} points redeemed, Ksh ${discount} off`);
  };
  ...
  return (
    <div className="grid grid-cols-12 gap-6">
          ...
          <div className="flex justify-between items-center">
            ...
            <div className="flex gap-2 items-center">
              <div className="relative">
                ...
                <input type="tel" placeholder="Phone number" value={customerPhone} onChange={e => setCustomerPhone(e.target.value)} className="pl-10 pr-2 py-1 border rounded-full text-sm w-36" />
                <button onClick={searchCustomer} className="ml-1 text-xs bg-amber-500 text-white px-2 py-1 rounded-full">Find</button>
                {customerSearchResult && <button onClick={clearCustomer} className="ml-1 text-xs text-red-500">X</button>}
              </div>
            </div>
          </div>
          {customerId && customerPoints > 0 && (
            <div className="mt-3 p-2 bg-amber-50 rounded-lg flex justify-between items-center">
              <span className="text-sm">Points: {customerPoints}</span>
              <div className="flex gap-2"><input type="number" placeholder="Redeem" value={redeemPoints} onChange={e => setRedeemPoints(parseInt(e.target.value) || 0)} className="w-20 p-1 border rounded text-sm" /><button onClick={applyRedeem} className="bg-amber-500 text-white px-2 py-1 rounded text-sm">Apply</button></div>
            </div>
          )}
        </div>
      ...
      {showReceipt && lastOrder && <ReceiptModal order={lastOrder} changeAmount={lastChange} discounts={appliedDiscounts} pointsDiscount={discountFromPoints} customer={customerSearchResult} onClose={() => setShowReceipt(false)} />}
    </div>
  );
}
